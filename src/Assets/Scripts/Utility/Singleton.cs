...
namespace Utility
{
    ...
    public class Singleton<T> : MonoBehaviour where T : Singleton<T>
    {
        ...
        private static bool nullWarningThrown = false;
...
#if UNITY_EDITOR
                if (instance == null && typeof(T).Name != "DebugConsoleHelper" && !nullWarningThrown)
                {
                    nullWarningThrown = true;
                    Debug.LogWarningFormat("singleton of type {0} was accessed but there is no instance in the scene.", typeof(T).Name);
                }
#endif
                return instance;
            }
